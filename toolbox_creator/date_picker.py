...
def popup_get_date(
    ...
    switch_color = (sg.theme_background_color(), sg.theme_text_color())
    ...
    if not close_when_chosen:
        layout += [
            [
                sg.Button("Ok", border_width=0, font="TkFixedFont 8"),
                sg.Button("Cancel", border_width=0, font="TkFixedFont 8"),
            ]
        ]

    window_date_picker = sg.Window(
        title,
        layout,
    ...
    update_days(window_date_picker, cur_month, cur_year, begin_at_sunday_plus)

    prev_choice = chosen_mon_day_year = None

    # for week in range(6):
    #     for day in range(7):
    #         window_date_picker[(week, day)].bind(
    #             "<Enter>",
    #             f"+MOUSE_OVER_{week}_{day}+",
    #             w=window_date_picker,
    #         )
    #         window_date_picker[(week, day)].bind(
    #             "<Leave>",
    #             f"+MOUSE_OVER_{week}_{day}+",
    ...
    if cur_day:
        ...
        for week in range(6):
            for day in range(7):
                if window_date_picker[(week, day)].DisplayText == str(cur_day):
                    window_date_picker[(week, day)].update(
                        background_color=sg.theme_text_color(),
                        text_color=sg.theme_background_color(),
                    )
    ...
    while True:  # Event Loop
        ...
        if event in (sg.WIN_CLOSED, "Cancel", None):
            if (
                ...
                chosen_mon_day_year = (start_mon, start_day, start_year)
            else:
                today = datetime.date.today()
                chosen_mon_day_year = (today.month, today.day, today.year)
            break
        if event == "Ok":
            break
        if event in ("-MON-UP-", "-MON-DOWN-", "-YEAR-UP-", "-YEAR-DOWN-"):
            ...
            if cur_month > 12:
                ...
                cur_month = 12
                cur_year -= 1
            window_date_picker["-MON-YEAR-"].update(
                "{} {}".format(mon_names[cur_month - 1], cur_year)
            )
            update_days(window_date_picker, cur_month, cur_year, begin_at_sunday_plus)
            if prev_choice:
                window_date_picker[prev_choice].update(
                    background_color=sg.theme_background_color(),
                    text_color=sg.theme_text_color(),
                )
        elif type(event) is tuple:
            try:
                if window_date_picker[event].DisplayText != "":
                    chosen_mon_day_year = (
                        cur_month,
                        int(window_date_picker[event].DisplayText),
                        cur_year,
                    )
                    if prev_choice:
                        window_date_picker[prev_choice].update(
                    ...
                    window_date_picker[event].update(
                        background_color=sg.theme_text_color(),
                        text_color=sg.theme_background_color(),
                    )
                    prev_choice = event
    ...
    window_date_picker.close()

    # yyyymmdd
    return (
        str(chosen_mon_day_year[2])
        + str(chosen_mon_day_year[0]).zfill(2)
        + str(chosen_mon_day_year[1]).zfill(2)
    )
    # return (chosen_mon_day_year[2], chosen_mon_day_year[0], chosen_mon_day_year[1])
